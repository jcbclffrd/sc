...
# Download ALL 375 samples from PRJNA729525
# This script processes the SRR list file
...
echo "================================================"
echo "PRJNA729525 Complete Dataset Download"
echo "Started: $(date)"
...
while read srr; do
    ...
    if [ -f "$DOWNLOAD_DIR/$srr/${srr}_1.fastq.gz" ] && [ -f "$DOWNLOAD_DIR/$srr/${srr}_2.fastq.gz" ]; then
    ...
    # Show summary
    if [ -f "$DOWNLOAD_DIR/$srr/${srr}_1.fastq.gz" ]; then
...
echo ""
echo "Summary:"
SUCCESS=$(find $DOWNLOAD_DIR -name "*_1.fastq.gz" | wc -l)
echo "  Successfully downloaded: $SUCCESS / $TOTAL samples"
